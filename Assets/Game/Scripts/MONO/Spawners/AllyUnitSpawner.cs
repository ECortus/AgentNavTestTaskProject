...
public class AllyUnitSpawner : MonoBehaviour
{
    ...
    private GameObject prefab => prefabsCollection.AllyPrefab;
    ...
    async void Activate()
    {
        ...
        while (_spawnerCancellationToken != null && GameManager.GameStarted)
        {
            if (_getMouseInput)
            {
                ...
                try
                {
                    if (AgentsStat.Instance.AllyAlive < maxCount)
                    {
                    ...
                    {
                        await UniTask.WaitUntil(() => AgentsStat.Instance.AllyAlive < maxCount, 
                            PlayerLoopTiming.Update, _spawnerCancellationToken.Token);
                    }
                }
                catch
