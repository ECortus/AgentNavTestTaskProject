using System;
using System.Collections;
using System.Collections.Generic;
using System.Linq;
using ModestTree;
using UnityEngine;

public abstract class BasePool<TC, T, TP> : MonoBehaviour
    where TC : Component
    where T : Component, new()
    where TP : Enum
{
    [Serializable]
    public struct PoolElement
    {
        public TP Type;
        public T Element;
    }

    [SerializeField] private PoolElement[] Elements;
    
    [Serializable]
    public class Pool
    {
        public List<T> List = new List<T>();
    }
    
    private int _typeCount = 0;
    private int TypeCount
    {
        get
        {
            if(_typeCount == 0)
            {
                _typeCount = Enum.GetNames(typeof(TP)).Length;
            }
            return _typeCount;
        }
    }
    
    private Pool[] Pools = Array.Empty<Pool>();
    
    private void CreatePools()
    {
        Pools = new Pool[TypeCount];
        for(int i = 0; i < TypeCount; i++)
        {
            Pools[i] = new Pool();
        }
    }
    
    public List<T> GetList(TP type)
    {
        if(Pools.Length < TypeCount)
        {
            CreatePools();
        }
        
        return Pools[GetIndexByType(type)].List;
    }

    T GetElementByType(TP type)
    {
        return Elements.First(x => x.Type.Equals(type)).Element;
    }
    
    public GameObject Insert(TP type, Vector3 pos, Quaternion rot = new Quaternion())
    {
        List<T> list = GetList(type);
        T item;
        
        for(int i = 0; i < list.Count; i++)
        {
            item = list[i];
            if(!item) continue;

            if(Condition(item))
            {
                InsertAction(item, pos, rot);
                return item.gameObject;
            }
        }

        T prefab = GetElementByType(type);
        if (!prefab) return null;

        T scr = Instantiate(prefab);
        InsertAction(scr, pos, rot);
        
        list.Add(scr);
        
        return scr.gameObject;
    }
    protected abstract void InsertAction(T obj, Vector3 pos, Quaternion rot);
    protected abstract bool Condition(T obj);

    int GetIndexByType(TP type)
    {
        int i = Convert.ToInt32(type);
        if(i == 0) return -1;

        i = (int)(Mathf.Log(i, 2));
        return i;
    }
}
